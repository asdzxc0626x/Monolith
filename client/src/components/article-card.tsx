...
export function ArticleCard({ post }: { post: PostMeta }) {
  return (
    <Link href={`/posts/${post.slug}`} className="group block">
      <article className="relative overflow-hidden rounded-lg border border-border/40 bg-card/30 backdrop-blur-sm transition-all duration-300 hover:border-border/70 hover:bg-card/50 hover:shadow-[0_8px_30px_rgba(0,0,0,0.3)] hover:-translate-y-[2px]">
        ...
        <div className="p-[20px] lg:p-[24px]">
          ...
          <h2 className="text-[18px] font-semibold tracking-[-0.01em] leading-snug text-foreground transition-colors duration-200 group-hover:text-foreground/90 lg:text-[20px]">{post.title}</h2>
          <p className="mt-[10px] text-[14px] leading-[1.7] text-muted-foreground line-clamp-2">{post.excerpt}</p>
          <div className="mt-[16px] flex items-center gap-[6px] text-[13px] text-muted-foreground/50 transition-colors duration-200 group-hover:text-muted-foreground">
            <span>阅读全文</span>
